...
import { handleLumaMailSignal } from "../luma-mail/handler.ts";

export function registerLumaMailTool(worker: Worker): void {
  worker.tool("processLumaEmailSignal", {
    title: "Process Luma Email Signal",
    description:
      "Call when a Notion Mail trigger receives an email that appears to be for a Luma event. Validates Luma markers and records the signal for the incoming email.",
    schema: j.object({
      from: j
        .string()
