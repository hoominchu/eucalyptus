...
Add `Health Samples`, `Health Hourly Buckets`, `Sleep Sessions`, and `Health Sync Runs` only once the real sync needs dedupe/debuggability beyond the aggregate fixture.
...
| `Import Key` | Rich text | `metric:stepCount` |
| `HealthKit Identifier` | Rich text | Example: `stepCount` |
...
| Property | Type | Notes |
| --- | --- | --- |
| `Name` | Title | `Sync - stepCount - 2026-05-16 11:30` |
| `Metric` | Relation to `Metric Catalog` |  |
| `Started At` | Date |  |
| `Completed At` | Date |  |
...
| `Anchor Ref` | Rich text | Store actual HealthKit anchor in backend if needed; Notion can store a pointer |
| `Error` | Rich text |  |
