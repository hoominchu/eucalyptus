...
The important boundary is that scheduling, slacking-score, rotation, validation, and fixture-normalization code never imports Notion page/property types. Only `src/health/notion/NotionHealthDataRepository.ts` knows about Notion API details.
...
The current v0 implementation uses Node's built-in `fetch` for the first Notion write slice, so it does not require `@notionhq/client`, `zod`, `dotenv`, or `p-limit` yet. Add dependencies only when the implementation needs them.
...
eucalyptus/
  ...
  src/
    index.ts
    worker/
      registerHealthImportTool.ts
      registerLumaMailTool.ts
    health/
      config.ts
      ...
      notion/
        ...
        upsertPage.ts
        queryByKey.ts
        dailySummaryMapper.ts
        workoutMapper.ts
        # Metric Catalog mapper currently lives in NotionHealthDataRepository.ts.
        workerDecisionMapper.ts
    luma-mail/
      handler.ts
```
...
| --- | --- | --- | --- |
| `Daily Summary` | `Date` | Date | Stable upsert key, one row per local day |
| `Workouts` | `HK Workout UUID` | Rich text | Stable upsert key for Apple Health workouts |
| `Metric Catalog` | `HealthKit Identifier` | Rich text | Stable upsert key, e.g. `HKQuantityTypeIdentifierStepCount` |
...
Register tools through `src/worker/*` so `src/index.ts` only composes the Worker:

```ts
// src/worker/registerHealthImportTool.ts
...
export function registerHealthImportTool(worker: Worker): void {
  worker.tool("importHealthFixture", {
    ...
    schema: j.object({
      ...
      only: j.array(j.enum("metricCatalog", "dailySummaries", "workouts")).nullable()
    }),
...
1. `Metric Catalog`: independent lookup table; upsert by `HealthKit Identifier`.
2. `Daily Summary`: one row per local day; upsert by `Date`; save `dailyImportKey -> Notion page ID`.
...
2. Query the target data source for a row with its stable key (`HealthKit Identifier`, `Date`, `HK Workout UUID`, or `Import Key` depending on table).
...
Current first-write scope is `Metric Catalog` only:

```bash
ntn workers exec importHealthFixture \
  --local \
  --dotenv .env.local \
  -d '{"path":"./data/apple-health.fixture.json","dryRun":false,"only":["metricCatalog"]}'
```

Expected: `Metric Catalog` rows are created or updated by `HealthKit Identifier`.
...
ntn workers exec importHealthFixture \
  --local \
  --dotenv .env.local \
  -d '{"path":"./data/apple-health.fixture.json","dryRun":true,"only":["dailySummaries"]}'
```

Expected: only `Daily Summary` records are normalized; no Notion rows change.
...
The deployed Worker is named `eucalyptus`. The hosted tool should not depend on a laptop-local file path. The current path is `importHealthSyncRun`, which reads a mobile-uploaded Apple Health payload from a `Health Sync Runs` Notion row.
...
3. Should the current mobile upload server move behind a hosted webhook once the Worker webhook shape is clear?
4. Should future large health exports move from Notion page chunks to object storage?
