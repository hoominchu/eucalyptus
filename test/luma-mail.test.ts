...
import { handleLumaMailSignal, isLumaEmail } from "../src/luma-mail/handler.ts";
...
test("matches lu.ma links in the email URL", () => {
  assert.equal(
    isLumaEmail({
      from: "host@example.com",
      subject: "Join us",
...
test("logs handled marker for Luma email", () => {
  const logs: unknown[] = [];
  const result = handleLumaMailSignal(
    {
